...
from evaluation.paper_util import display_names, option_names, vibrant_colour_map, get_font_color
...
def aggregate_predictions(results):
    ...
    # aggregated_biomarker_results['options'] = [results.iloc[0]['options']]*len(aggregated_biomarker_results)
...
def create_bar_plot(dataframe, title):
    palette = vibrant_colour_map

    # Reshape the DataFrame into a long format
    df_long = dataframe.reindex(palette.keys()).reset_index().melt(id_vars='model_display_name', var_name='Option', value_name='Value').copy()
    df_long['Value'] = df_long['Value'].apply(lambda x: max(3, x))

    # Create the plot
    plt.figure(figsize=(14, 4))
    barplot = sns.barplot(x='Option', y='Value', hue='model_display_name', data=df_long, palette=vibrant_colour_map, alpha=1)
    ...
    # Show the plot
    plt.tight_layout(rect=[0, 0, 0.9, 1])  # Adjust the rect to make space for the legend
    plt.show()
...
class ClosedEndedFigures():
    def __init__(self, config, all_results, figure_path):
        ...
        self.colors = [self.config.dataset.task.colors[m] for m in [self.config.dataset.task.models.trainee_model[-1], self.config.dataset.task.models.specialist_model[-1], self.config.dataset.task.models.retinal_specialists[-1]]]
    ...
    def paper_f1_statistics(self):
        ...
        # biomarker_results = res.loc['SpecialistDetection'].groupby('model_display_name')
        # intermediate_results = pd.DataFrame(index=res.index)
        # intermediate_results['Comparison Model'] = intermediate_results.index.to_series().apply(
        #     lambda n: self.config.dataset.task.trainee_model if 'Specialist' in n else 
        #             self.config.dataset.task.baseline_model if 'Trainee' in n else None
        # )

        def present_ci(r):
            if pd.isna(r):
                return ''
            return f'{self.round100(r[0])} ({self.round100(r[1][0])}, {self.round100(r[1][1])})'

        def comparison(name):
            if 'Junior' in name:
                return self.config.dataset.task.models.specialist_model[-1]
            elif 'Specialist' in name:
                return self.config.dataset.task.models.trainee_model[-1]
            elif 'Trainee' in name:
                return self.config.dataset.task.models.baseline_models[-1]
            elif 'LLaVA-Med' in name:
                return self.config.dataset.task.models.baseline_models[-2]    
        ...
        (100 * staging_significance_df).to_csv(os.path.join(self.staging_path, 'pairwise_significance.csv'))
        ...
        (100 * referral_significance_df).to_csv(os.path.join(self.referral_three_levels_path, 'pairwise_significance.csv'))
        ...
        if 'SpecialistDetection' in self.all_results['task_type']:
            ...
            # aggregated_biomarker_results['Comparison Model'] = aggregated_biomarker_results.index.to_series().apply(comparison)
            # table['Biomarker p-value (McNemars)'] = aggregated_biomarker_results.apply(lambda p: mcnemars_test(p[0], aggregated_biomarker_results.loc[p[-1]]['predictions'], p[1]) if p[-1] is not None else None, axis=1)
            # show_table['Biomarker p-value (McNemars)'] = table['Biomarker p-value (McNemars)'].apply(self.round1)
    ...
    def biomarker_detection_per_severity(self, model, cot):
        ...
        for task, severity in task_severity_pairs:
            ...
            accuracy_by_severity['Sensitivity'] *= 100
    ...
    def model_comparison_barplots(self, f1_table, experiment):
        ...
        barplot_df['F1'] =      (100 * df.apply(lambda x: x[0])   ).apply(lambda x: max(x, 1))
        barplot_df['lower'] =   (100 * df.apply(lambda x: x[1][0])).apply(lambda x: max(x, 1))
        barplot_df['upper'] =   (100 * df.apply(lambda x: x[1][1])).apply(lambda x: max(x, 1))
        ...
        # barplot_df['color'] = barplot_df['model_display_name'].map(colors_dict)
        ...
        ax.set_ylim(-2, min((barplot_df['F1'] + barplot_df['yerr_upper']).max() + 2, 100))
        ...
        # Add a legend with model names and their colors
        # handles = [plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=colors_dict[model], markersize=10) 
        #         for model in self.config.dataset.task.display_names.values() if model in colors_dict.keys()]
        # plt.legend(handles, self.config.dataset.task.display_names.values(), title='Models', bbox_to_anchor=(0.5, -0.15), loc='upper center', ncol=2)

        plt.legend([], [], frameon=False)  # Remove the legend added by hue
        return ax

    def radar(self, data):
        plot_data = data.map(lambda x: max(1, x))
        ...
        for (index, d), marker in zip(plot_data.reindex(colors.keys()).iterrows(), markers):
            ...
            # ax.plot(theta, d, label=index, color=color, lw=4, marker=marker, markersize=12)  # Use the same color for each line
            ...
            # if index == self.config.dataset.task.models.specialist_model[-1]:
            #     for i, value in enumerate(d[:-1]):
            #         ax.text(theta[i], value, f'{value:.2f}', horizontalalignment='center', size=12, color='black')
    ...
    def closed_ended_figures(self):
        ...
        self.referral_two_levels_path = os.path.join(self.cot_path, 'referral_two_levels')
        ...
        # for referral_levels, task in [(2, self.config.dataset.task.referral_two_levels_task), (3, self.config.dataset.task.referral_three_levels_task)]:
        ...
        staging_f1 = staging_f1.apply(lambda r: 100 * r)
        ...
        referral_three_levels_f1 = referral_three_levels_f1.apply(lambda r: 100 * r)
        ...
        for junior in jrp_staging.values():
            junior_f1 = 100 * bootstrap_f1_confidence_interval(junior['labels'], junior['predictions'], self.config.dataset.task.staging_task_positive, positive_classes=self.config.dataset.task.staging_task_positive, n_bootstraps=0, ci=95)
        ...
        #       jitter=True, size=8, edgecolor='black', linewidth=1.5, facecolors='white')

        plt.xticks([])
        ax.set_ylim(ax.get_ylim()[0], max(ax.get_ylim()[1], max(junior_ophthalmologists_scores)) + 1)

        save_fig_path_creation(os.path.join(self.staging_path, f'Model_comparison_barplot.png'))

        # Referral results
        # self.model_comparison_barplots(f1_table, 'Referral (Two Levels) F1')
        # plt.ylabel('Patient referral F1 Score')
        # save_fig_path_creation(os.path.join(self.referral_two_levels_path, f'Model_comparison_barplot.png'))
        ...
        for junior in jrp_referral.values():
            junior_f1 = 100 * bootstrap_f1_confidence_interval(junior['labels'], junior['predictions'], ['not', 'next year', 'next two weeks'], positive_classes=self.config.dataset.task.referral_three_levels_positive, n_bootstraps=0, ci=95)
            junior_ophthalmologists_scores.append(junior_f1) # Replace with actual values
        ...
        if self.all_results['task_type'].apply(lambda x: x == 'SpecialistDetection').any():
            ...
            biomarker_f1 = biomarker_f1.apply(lambda r: 100 * r)
        ...
        for model in models:
            ...
            if model in staging_res.index:
                ...
                plt.figure(figsize=(10, 8.57))
                # sns.heatmap(np.array(res['result'])[:-1], annot=True, xticklabels=res['options'] + ['Invalid response'], yticklabels=res['options'], cmap=self.cmap, cbar=False, vmax=22)
            ...
            # referral_res = self.results_ixed.loc[cot].loc['SpecialistReferral'].loc[self.config.dataset.task.referral_two_levels_task]
            # if model in referral_res.index:
            #     res = referral_res.loc[model]
            #     plt.figure(figsize=(12, 12))
            #     sns.heatmap(np.array(res['result'])[:-1], annot=True, xticklabels=res['options'] + ['Invalid response'], yticklabels=res['options'], cmap=self.cmap, cbar=False, vmax=22)
            #     plt.xticks(rotation=45, ha='right', rotation_mode='anchor')
            #     save_fig_path_creation(os.path.join(self.referral_two_levels_path, f'{model} - Referral_heatmap.png'))
    ...
    def clinical_evaluation_figures(self):
        ...
        for clinician, study_df, results in [
                ('Andrew', 'evaluation/results/open_ended/16_06_GEN/Andrew_clinical_evaluations.csv', '/u/home/holr/Documents/data/specialist_annotations/clinical_evaluation/Clinical evaluation (completed Andrew).xlsx'),
                ('Sobha', 'evaluation/results/open_ended/16_06_GEN/Sobha_clinical_evaluations.csv', '/u/home/holr/Documents/data/specialist_annotations/clinical_evaluation/Clinical evaluation (Sobha - updated).xlsx')]:
        ...
        fig, ax = plt.subplots()
        [ax.scatter([], [], color=colors[i], label=labels[i], s=100, marker='s') for i in range(len(colors))]
        ...
        df.loc[df['ImageId'] == '40812321-25-BAXNLFCRFEKUAVKPNVMVHMCNL+ALSQCDCEHFRCUAUHGONSWQRCVKXPGVVNWZHCW+WIBGSQ.png']
        df.loc[df['ImageId'] == '16047443-25-VYZBXKAYPEPPAHARTTWHDBIDY+XNQZDTYIFKPGYLIUAELGGZWMDTNMILNFGHPLJ+XULOMJ.png']
        df.loc[df['ImageId'] == '16042115-25-HSIVKVKAPIXYRFLCMRCXRYKFS+XTEFQWVHNEHAUFFHDNFGCSYIYKYGNIJXWBRWF+UBKUSY.png']
        df.loc[df['ImageId'] == '16041764-25-TGQGHZCJCGJNWIXXOBQHBCITV+XGYHVFWYVTTUGTTXCITNRDIGVSKUHSVEIJLME+GRAWQS.png']
        
        x = 3

    def manual_save_load_predictions(self, task_type, task_name, model):
        self.manual_predictions_path = f'/u/home/holr/Documents/retinavlm/evaluation/results/closed_ended/manual_predictions/{task_type}/{task_name}/{model}'
        ...
        # manual_label_extractions = pd.read_csv('/u/home/holr/Documents/retinavlm/evaluation/figures/closed_ended/09_06_TEST_SET_biomarker_stage-09_06+_TEST_SET_biomarker_stage-10_06_TEST_SET_staging-09_06_TEST_SET_referral-09_06+_TEST_SET_referral/prediction_extraction_manual.csv')
...
def generate_figures(config):
    # figure_path = os.path.join(config.finished_closed_ended_results, 'figures')
    ...
    # needs_retake = ['153_A.png', '78_C.png', '61_A.png']
    # all_results.loc[all_results['task_name'] == config.dataset.task.referral_three_levels_task]
    # for i, row in all_results.loc[all_results['task_name'] == config.dataset.task.referral_three_levels_task].iterrows():
    #     all_results.loc[i]['predictions'] = 3
    ...
    # figures.manual_save_load_predictions('SpecialistOther', 'AMDStage', 'Specialist-RetinaVLM (V4.53 192)')
    
    figures.manual_save_load_predictions('SpecialistReferral', 'SouthamptonReferral', 'Specialist-RetinaVLM (V4.52 192)')
    # figures.manual_save_load_predictions('SpecialistReferral', 'SouthamptonReferral', 'Specialist-RetinaVLM (V4.53 192)')

    # specialist_referral = figures.results_ixed.loc[True].loc['SpecialistReferral'].loc['AttentionTreatment'].loc[config.dataset.task.models.specialist_model[-1]]
    # comparison_df = pd.DataFrame({k: specialist_referral[k] for k in ['ImageId', 'labels', 'predictions', 'inputs', 'outputs']})
    # comparison_df['inputs'] = comparison_df['inputs'].apply(lambda x: x.split('assistant<|end_header_id|>\n\n')[-1])

    # comparison_df.loc[(comparison_df['labels'] == 'attention')]['inputs'].tolist()

    figures.closed_ended_figures()
    # clinical_evaluation_figures(config, figure_path)
    
    result = figures.results_ixed.loc[True].loc['SpecialistReferral'].loc[config.dataset.task.referral_three_levels_task].loc['Specialist-RetinaVLM (V4.52 192)']
    ...
    result = figures.results_ixed.loc[True].loc['SpecialistOther'].loc[config.dataset.task.staging_task].loc['Specialist-RetinaVLM (V4.52 192)']
    comparison_df = pd.DataFrame({k: result[k] for k in ['labels', 'predictions', 'inputs', 'outputs']})
    comparison_df['ImageId'] = result['ImageId']
